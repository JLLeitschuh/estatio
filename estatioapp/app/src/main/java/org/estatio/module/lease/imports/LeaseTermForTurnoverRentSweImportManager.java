...
public class LeaseTermForTurnoverRentSweImportManager {
            ...
            torItems.forEach(tor->{
                ...
                tor.getTerms().forEach(torTerm -> {
                    if (torTerm.getStartDate().getYear()==getYear()-1) {
                        ...
                        line.setPercentagePreviousYear(percentageFromTurnoverRentRule(term.getTurnoverRentRule()));
                    }
                    if (torTerm.getStartDate().getYear()==getYear()) {
                        ...
                        line.setPercentage(percentageFromTurnoverRentRule(term.getTurnoverRentRule()));
                    }
                });
                // every item should produce a line since autocreate is turned off (ECP-806)
                // TODO: ECP-1107 NO LONGER TRUE NOW, because we use regular turnover rent terms again ==> so what is the consequence of this???
    ...
    @Programmatic
    public static BigDecimal percentageFromTurnoverRentRule(final String turnoverRentRule){
        String[] rules;
        if (turnoverRentRule != null && turnoverRentRule.trim().length() != 0) {
            rules = turnoverRentRule.split(";");
            return new BigDecimal(rules[0]);
        }
        return null;
    }
